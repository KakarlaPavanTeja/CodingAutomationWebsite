...
- **Testcase generator → Gemini 2.5 Pro at max thinking (why):** `generate_testcases` runs at the highest reasoning effort. OpenAI reasoning models (e.g. `gpt-5.4`) **hide** their reasoning, so during the think phase they stream **zero bytes**; that silent socket outlasts the proxy gateway's idle timeout and the connection is severed mid-stream → `finish_reason=None`, empty content, the old "LLM returned empty content" failure on hard problems. **Gemini 2.5 Pro streams its reasoning tokens as it thinks**, so the socket stays warm even at full effort and the gateway never cuts it — same top-tier quality, ~$0.40/run. Reasoning stays `high` (Gemini's max thinking budget); the `testcases` output cap (80K) covers thinking-as-completion tokens + the generator script. Env overrides intact: `OPENROUTER_MODEL_TESTCASES`, `OPENAI_REASONING_EFFORT_TESTCASES`, `OPENAI_MAX_TOKENS_TESTCASES`. **Self-healing net:** if a testcases call still returns empty content with `finish_reason != "length"` (a severed silent connection, not budget exhaustion), `call_llm` retries once at the next-lower reasoning effort before raising; the `finish_reason == "length"` (budget) case still raises loudly.
