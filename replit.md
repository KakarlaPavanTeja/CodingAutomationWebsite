...
- **Gateway WAF blocker (`split_code`):** the proxy gateway runs an OWASP-CRS-style WAF that returns an HTML `403 Forbidden` when the request body contains Java-RCE class signatures — confirmed: `java.io.*` (File/FileWriter/IOException/BufferedReader) and `java.lang.Runtime`. The code-splitting system prompt (`Prompts/splittingPrompt.py`) embeds a Java driver template with five `java.io.*` imports, sent on **every** `split_code` call regardless of target language, so every call is blocked. This 403 is **permanent/content-based, not transient**: `llm_client._is_waf_block()` detects the HTML body and fails fast with an actionable error (bounded retry, env `OPENROUTER_GATEWAY_403_RETRIES`, is reserved for JSON 403s only). **The real fix is gateway-side: whitelist/disable the OWASP-CRS Java RCE rule (944xxx family) for this proxy, or allowlist the pipeline's traffic.** Editing the prompt to dodge the signature is fragile and won't help when a user's own Java solution legitimately uses `java.io.*`.
