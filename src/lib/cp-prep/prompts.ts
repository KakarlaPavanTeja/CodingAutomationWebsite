...
- Missing/garbled constraints: never leave blank. In PORT MODE infer bounds from the reference solution's data types and the algorithm's complexity (an int value caps near 2e9 -> write ≤ 10^9 by convention; a long long value/sum/answer/return type signals values exceed int range -> write ≤ 10^18). In AUTHOR MODE you have no reference types, so infer from the example magnitudes, the operation described (does it sum values? the answer may exceed int range), and standard CP convention for the apparent difficulty. Either way: an O(n) or O(n log n) solution handles n up to ~1e5–1e6; pick a clean round bound, and prefer the larger, safer bound when in doubt. Write inferred bounds as plain constraint bullets (e.g. 1 ≤ n ≤ 10^5) with no editorial meta-notes — never append "(inferred; ...)" or similar asides in the statement. Document what you inferred and on what basis in the report only.
