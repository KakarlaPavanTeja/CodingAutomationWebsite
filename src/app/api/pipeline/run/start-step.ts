...
import { buildCommand, getStepConfig, getAllTrackedStepIds } from "@/lib/pipeline-config";
...
export async function startStep(args: StartStepArgs): Promise<StartStepResult> {
  ...
  const { script, args: cmdArgs } = buildCommand(
    stepId,
    effectiveMode,
    subSteps ?? [],
    languages ?? [],
  ...
  const logStepKey = pipelineRunLogKey(stepId, subSteps, runKey);
  ...
  const proc = spawn(pythonPath, [scriptPath, ...cmdArgs], {
    cwd: tmpDir,
