"""
temp_execution_manager.py — batch-submit + poll runner for the NEW compiler.
===========================================================================
Experimental sibling of execution_manager_v2.py.
...
Designed for VERY LARGE testcases sent all at once: IO is inlined as plain
text (no S3, no base64 — base64 would only inflate an already-large payload).

Usage (same shape as execution_manager_v2):
    python temp_execution_manager.py                     # all supported langs, function-based
    python temp_execution_manager.py python              # single language
    python temp_execution_manager.py cpp java            # multiple languages
    python temp_execution_manager.py --nonfunction       # single-file (generatedFullCode)
    python temp_execution_manager.py --url http://localhost:8000   # local orchestrator
...
def _build_testcases_payload(testcases):
    ...
    {test_index, order, input, expected} so batch results can be matched back.
    """
    payload_testcases = []
    id_index = {}
    ...
    return payload_testcases, id_index
...
def submit_compile(base_url, lang_id, main_file, files_payload, payload_testcases,
                   default_time_limit):
    payload = {
        "language": lang_id,
        "files": files_payload,
        ...
        "testcases": payload_testcases,
    }
    url = f"{base_url}/compile"
    start = time.perf_counter()
    resp = requests.post(url, json=payload, timeout=SUBMIT_TIMEOUT_SECONDS)
...
def _run_one_language(base_dir, lang, code_files, main_file, lang_id,
    ...
    print(f"Submitting {len(payload_testcases)} testcases in one request "
          f"(~{total_bytes / 1024:.1f} KB inline) to {NEW_COMPILER_URL}/compile", flush=True)

    try:
        submit_resp, submit_ms = submit_compile(
            NEW_COMPILER_URL, lang_id, main_file, files_payload,
            payload_testcases, default_time_limit,
        )
    ...
    for tr in language_results:
        _print_progress(
            ...
            error_details=(tr.get("error") or "")[:180] if not tr["passed"] else "",
        )
...
def run_all_tests_batch(base_dir=None, testcases_path=None, selected_lang=None,
    ...
    if selected_lang == "__multi__" or (selected_lang and len(target_languages) == 1):
    ...
    for lang in target_languages:
        ...
        try:
            language_results, passed_count, halted = _run_one_language(
                ...
                config.get("default_execution_time_limit", 5), testcases, question_id,
            )
...
def main():
    ...
    print("\n[temp_execution_manager] NEW compiler — batch submit + poll")
    ...
    print(f"\n{'=' * 80}")
    print("SUMMARY (temp_execution_manager — batch)")
    print(f"{'=' * 80}")
    summary_rows = []
